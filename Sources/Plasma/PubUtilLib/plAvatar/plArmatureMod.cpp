...
{
    plString strBuf = plString::Format("Armature <%s>:", fRootName.c_str());
    ...
    if(SO)
    {
        ...
        strBuf = plString::Format("position(world): %.2f, %.2f, %.2f Opaque: %3s",
        ...
        strBuf = plString::Format("In world: %s  Frozen: %s", world ? world->GetName().c_str() : "nil", frozen);
        debugTxt.DrawString(x,y, strBuf);
        y+= lineHeight;
        ...
        if (fController)
        {
            ...
            strBuf = plString::Format("position(physical): <%.2f, %.2f, %.2f> velocity: <%5.2f, %5.2f, %5.2f>",
                        physPos.fX, physPos.fY, physPos.fZ, vel.fX, vel.fY, vel.fZ);
        }
        else
        {
            strBuf = "position(physical): no controller";
        }
        debugTxt.DrawString(x, y, strBuf);
        y += lineHeight;
    }
    ...
    if (fClothingOutfit)
    {
        ...
        {
            strBuf += plString::Format("%s%s",
                        (itemCount == 0) ? "    " : "",
                        fClothingOutfit->fItems[i]->fName);
    ...
    if (plRelevanceMgr::Instance()->GetEnabled())
    {
        ...
        y += lineHeight;
        strBuf = plString::Format("          In: %s", plRelevanceMgr::Instance()->GetRegionNames(fRegionsImIn).c_str());
        debugTxt.DrawString(x, y, strBuf);
        y += lineHeight;
        strBuf = plString::Format("  Care about: %s", plRelevanceMgr::Instance()->GetRegionNames(fRegionsICareAbout).c_str());
        debugTxt.DrawString(x, y, strBuf);
        y += lineHeight;
    }
}
...
{
    plString buff;
    buff = plString::Format("Mouse Input Map: %s", plAvatarInputInterface::GetInstance()->GetInputMapName());
    debugTxt.DrawString(x, y, buff);
    y += lineHeight;

    buff = plString::Format("Turn strength: %.2f (key: %.2f, analog: %.2f)", GetTurnStrength(), GetKeyTurnStrength(), GetAnalogTurnStrength());
    debugTxt.DrawString(x, y, buff);
    y += lineHeight;
...
plString plArmatureMod::GetMoveKeyString()
{
    plStringStream buff;
    buff << "Move keys: ";

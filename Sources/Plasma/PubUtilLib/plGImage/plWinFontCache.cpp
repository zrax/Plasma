...
HFONT   plWinFontCache::IMakeFont( const plString &face, int height, int weight, bool italic, uint32_t quality )
{
    ...
    myRec.fFont = CreateFontW( height, 0, 0, 0, weight, italic ? TRUE : FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
                               CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, VARIABLE_PITCH, face.ToWchar() );
...
//#ifdef HS_DEBUGGING
        ...
        if( GetObjectW( myRec.fFont, sizeof( fontInfo ), &fontInfo ) )
        {
            const char *err = nil;
            ...
            if( face.CompareI(plString::FromWchar(fontInfo.lfFaceName)) != 0 )
    ...
    {
        plStatusLog::AddLineS( "pipeline.log", "ERROR: CreateFont() call FAILED (face: %s, size: %d %s %s)", face.c_str(), -height, weight == FW_BOLD ? "bold" : "", italic ? "italic" : "" );
    }

...
bool plPythonFileMod::ILoadPythonCode()
{
    ...
    if (plFileInfo(pyfile).Exists())
    {
        ...
        DisplayPythonOutput();
        char errMsg[256];
        sprintf(errMsg, "Python file %s.py had errors!!! Could not load.", fPythonFile.c_str());
    ...
    DisplayPythonOutput();
    char errMsg[256];
    sprintf(errMsg, "Python file %s.py was not found.", fPythonFile.c_str());
    PythonInterface::WriteToLog(errMsg);
    hsAssert(0,errMsg);
    return false;
}
    ...
    if ( !fAtConvertTime )      // if this is just an Add that's during a convert, then don't do anymore
    {
        ...
        if (!fPythonFile.IsEmpty())
        {
            ...
            // set the name of the file (in the global dictionary of the module)
                ...
                PyObject* pfilename = PyString_FromStringEx(fPythonFile);
            ...
            //  - create instance of class
                ...
                if ( fInstance == nil )     // then there was an error
                {
                    // display any output (NOTE: this would be disabled in production)
                    char errMsg[256];
                    sprintf(errMsg, "Python file %s.py, instance not found.", fPythonFile.c_str());
                    PythonInterface::WriteToLog(errMsg);
                    hsAssert(0, errMsg);
